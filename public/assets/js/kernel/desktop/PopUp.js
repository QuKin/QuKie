...
let { PopUpL } = await import(
  '../../language/' + getConfig('Language') + '/kernel/desktop/PopUpL.js'
)
...
export default class PopUp {
   ...
   * ```
   */
  constructor(title, options, range = 't', typeColor = 'info') {
    this.qrange = new QRange(
      't',
      'l',
      'r',
      'b',
  ...
  init(title, options, range, typeColor) {
    this.setTitle(title)
  ...
  setRange(range) {
    if (this.qrange.is(range)) {
      ...
      return QAL(
        ...
        publicL.RangeError,
        [],
        PopUpL.rangeErrorOnlySupported + ':' + this.qrange.show(),
        CodeE.RangeError,
      )
    }
  }
          ...
          switch (length) {
            ...
            default:
              return QAL(
                window.LogIntensityE.Error,
                PopUpL.type,
                PopUpL.setTypeColorError,
                [],
                'border参数错误，border参数只能一个颜色，或者三个整体参数：1px solid red',

...
let { QDBL } = await import(
  '../../language/' +
    getConfig('Language') +
    '/kernel/systemCallInterface/QDBL.js'
)
...
export default class QDB {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = function (event) {
        ...
        for (const store of _this.stores) {
          let objectStore = _this.db.createObjectStore(
            store.name,
            store.options,
          )
  ...
  setStoreName(storeName) {
    ...
    let b = this.qrange.is(storeName)
    if (b) {
      this.storeName = storeName
      return QAL(
        window.LogIntensityE.SuccessError,
        QDBL.type,
        QDBL.setStoreNameSuccess,
        this.storeName,
      )
    } else {
      return QAL(
  ...
  close() {
    return new Promise((resolve, reject) => {
      try {
        this.db.close()
        resolve(
          QAL(window.LogIntensityE.SuccessError, QDBL.type, QDBL.closeSuccess),
        )
      } catch (e) {
        reject(
          QAL(
   ...
   * @returns {Promise}
   */
  page(indexName, indexValue, page, pageSize) {
    return new Promise((resolve, reject) => {
      if (isEmptyValue(indexName).data || isEmptyValue(indexValue).data) {
        reject(
          QAL(

 ...
 * @Date            2023/8/7 9:33
 */
let { QJsonL } = await import(
  '../../language/' +
    getConfig('Language') +
    '/kernel/systemCallInterface/QJsonL.js'
)
export default class QJson {
  ...
  set(json) {
    if (typeof json === 'string') {
      if (json.indexOf('.json') === -1) {
        ...
        console.log(QJsonL.nodeInOrderToImportJson)
      }
    } else {
      this.json = json
    }
  ...
  whereAll(arr, join = '&&') {
    ...
    this.json.forEach((value) => {
      ...
      for (const item of temp) {
        a = !isNaN(Number(value[item[0]]))
          ? parseInt(value[item[0]])
          : '"' + value[item[0]] + '"'
  ...
  findDelete(json, path) {
    for (const i in json) {
      if (json[i] != null && typeof json[i] == 'object')
        this.findDelete(json[i], path.concat(i))
  ...
  innerJoin(json, key) {
    ...
    // 获取两个json所有的key值，并去重
    let keys = [
      ...new Set(Object.keys(this.json[0]).concat(Object.keys(json[0]))),
    ]
      ...
      for (let j = 0; j < json.length; j++) {
        ...
        if (typeof key === 'string') {
          ...
          for (let k = 0; k < key.length; k++) {
            ifEval +=
              'this.json[i]["' + key[k] + '"]===json[j]["' + key[k] + '"] && '
          }
      ...
      for (let j = 0; j < this.json.length; j++) {
        if (typeof key === 'string') {
          if (json[i][key] === this.json[j][key]) {
            ...
            for (let o = 0; o < keys.length; o++) {
              temp[keys[o]] =
                json[i][keys[o]] === undefined ? null : json[i][keys[o]]
            }
          }
        } else if (typeof key === 'object') {
          let ifEval = ''
          for (let k = 0; k < key.length; k++) {
            ifEval +=
              'json[i]["' + key[k] + '"]===this.json[j]["' + key[k] + '"] && '
          }
          ifEval += '1===1'
          if (eval(ifEval)) {
            for (let o = 0; o < keys.length; o++) {
